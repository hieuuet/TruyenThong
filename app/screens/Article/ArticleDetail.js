...
class ArticleDetail extends Component {
    ...
    componentWillMount() {
        this._getNewsDetail(this.props.navigation.state.params.id);
    }
        ...
        return (
            <View style={styles.articleContainer}>
                ...
                <ScrollView keyboardShouldPersistTaps={'handled'}>
                    <View style={{flex: 1}}>
                        <Image style={styles.postCover} source={{uri: this.state.newsDetail.img}} resizeMode="cover" />

                        <Text style={styles.date}>{this.state.newsDetail.publish_date} - {this.state.newsDetail.area_name}</Text>
                        <Text style={styles.title}>{this.state.newsDetail.title}</Text>
                        <Text style={styles.shortDes}>{this.state.newsDetail.short_description}</Text>
...
export default connect(mapStateToProps, actionCreators)(ArticleDetail);
