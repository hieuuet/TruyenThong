...
class PayPostpaidMobile extends Component {
    ...
    constructor(props){
        ...
        const providerOptions = [
            {value: 'VMSBILLING', label: 'Di động MobiFone trả sau', des: 'Thanh toán di động MobiFone trả sau', type: 'BILLING'},
            {value: 'VNPBILLING',label: 'Di động Vinaphone trả sau', des: 'Thanh toán di động Vinaphone trả sau', type: 'BILLING'},
            {value: 'VTLBILLING', label: 'Di động Viettel trả sau', des: 'Thanh toán di động Viettel trả sau', type: 'BILLING'},
            {value: 'VTLHP', label: 'Viettel Homephone', des: 'Thanh toán điện thoại Viettel Homephone', type: 'BILLING'},
            {value: 'VNPTHNBILLING', label: 'Điện thoại cố định VNPT Hà Nội', des: 'Thanh toán ĐTCĐ VNPT Hà Nội', type: 'BILLING'},
            {value: 'VNPTHCMBILLING', label: 'Điện thoại cố định VNPT Hồ Chí Minh', des: 'Thanh toán ĐTCĐ VNPT Hồ Chí Minh', type: 'BILLING'},
            {value: 'SSTHPBILL', label: 'Điện thoại cố định Nam Sài Gòn', des: 'Thanh toán ĐTCĐ VNPT Nam Sài Gòn', type: 'BILLING'},
            {value: 'STCBILLING', label: 'Điện thoại cố định SPT', des: 'Thanh toán ĐTCĐ SPT', type: 'BILLING'}
        ];
    ...
    onPayBill = () => {
      Util.showConfirm('Bạn có chắc chắn muốn thanh toán hóa đơn điện thoại trả sau?\nSố tiền: ' + this.state.formattedAmount, this.onPay.bind(this));
    }
    ...
    onCheckBill = () => {
        ...
        this.setState({loading : true});
        Api.checkBill(serviceCode, '0' + billingCode).then(res => {
            this.setState({loading : false});
            if (res.code === 'SUCCESS') {
            ...
            <View style={styles.container}>
                ...
                <ScrollView style={{zIndex: 5}} keyboardShouldPersistTaps={'handled'}>
                    <View style={styles.header}>
                        ...
                        <Image source={require('../../images/utilities/small/dienthoaitrasau.png')} resizeMode="contain" style={styles.coverIcon} />
                        <Text style={styles.pageTitle}>HÓA ĐƠN ĐIỆN THOẠI TRẢ SAU</Text>
                    ...
                    <View style={[globalStyles.contentCard]}>
                        ...
                        <View style={globalStyles.inputWrap}>
                            <TextInput returnKeyType='done' keyboardType={'phone-pad'} autoCapitalize={'none'} onChangeText={value => this.setState({billingCode: value, amount: 0, formattedAmount: '', additionalData : ''})} style={globalStyles.lineInput} editable={true} />
...
export default PayPostpaidMobile;
